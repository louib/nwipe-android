...
public class WipeAsyncTask extends AsyncTask<WipeJob, WipeJob, WipeJob> {
  ...
  public void executeWipePass() {
    ...
    // Log.i("MainActivity", String.format("Got %d bytes available for writing.",
    // availableBytesCount));
    ...
    try (OutputStream fos = this.getOutputStream(wipeFileName)) {
      ...
      if (e.toString().contains("ENOSP")
          && wipeJob.getCurrentPassPercentageCompletion() >= WipeJob.MIN_PERCENTAGE_COMPLETION) {
    ...
    try (InputStream fis = this.getInputStream(wipeFileName)) {
      ...
      while (this.wipeJob.wipedBytes < this.wipeJob.totalBytes) {
        ...
        if (!Arrays.equals(
            Arrays.copyOfRange(bytesBuffer, 0, bytesToReadCount),
            Arrays.copyOfRange(bytesInputBuffer, 0, bytesToReadCount))) {
  ...
  /** This function is required when mocking the class during unit testing. */
